 ...
 * @version $Id: ActionPane.java,v 1.1.4.1 2008/10/16 04:27:06 akurzhan Exp $
 */
public final class ActionPane extends JPanel {
    ...
    public ActionPane(TreePane tpane, ContainerHWC ctnr) {
        ...
        if ((mySystem != null) && (mySystem.getMyNetwork() != null))
        	cfgErrors = mySystem.getMyNetwork().getConfigurationErrors();
	...
	private class errorsTableModel extends AbstractTableModel {
		...
		public String getColumnName(int col) {
	        String buf = null;
	        switch(col) {
	        case 0:	buf = "Network Element"; break;
	        case 1: buf = "Error Message"; break;
	        }
			return buf;
	    }
		...
		public Object getValueAt(int row, int column) {
			if ((row < 0) || (row >= cfgErrors.size()) || (column < 0) || (column > 1))
				return null;
			if (column == 0)
				return cfgErrors.get(row).getSource();
