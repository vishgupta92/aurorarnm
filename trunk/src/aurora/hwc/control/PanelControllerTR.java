...
public final class PanelControllerTR extends AbstractSimpleControllerPanel {
	...
	private class TRTableModel extends AbstractTableModel {
		...
		public Object getValueAt(int row, int column) {
			if ((row < 0) || (row > t_dns.size() - 1) || (column < 0) || (column > 4) )
				return null;
		...
		public void setValueAt(Object value, int row, int column) {
			if ((row < 0) || (row >= t_dns.size()) || (column == 0))
				return;
			...
			try {
				...
				if ((selected != null) && (selected.length > 0))
					for (int i = 0; i < selected.length; i++) {
						...
						if ((idx >= 0) && (idx < t_dns.size()))
							trtablemodel.deleterow(idx);
					}
	    	}
	    	catch(Exception e) { }
	    	return;
		...
		private class ButtonEventsListener implements ActionListener {
			public void actionPerformed(ActionEvent e) {
				...
				if (cmdOK.equals(cmd)) {
					...
					if (toAdd)
						trtablemodel.addrow(l_id, l_dns, l_flw, l_spd, l_rate);
					else
