...
/**
 * Implementation of network internal frame for prediction.
 * @author Alex Kurzhanskiy
 * @version $Id: $
 */
public final class WindowNetworkP extends JInternalFrame implements ActionListener {
	private static final long serialVersionUID = -5952147819641321619L;
	...
	public WindowNetworkP() { }
	public WindowNetworkP(AbstractContainer ctnr, AbstractNodeComplex ne, TreePane tpane) {
	...
	private void saveSimulationData() {
		...
		if (fos == null)
			return;
		fos.println("\"Time\", \"Best Delay (vh)\", \"Worst Delay (vh)\"");
		...
		for (int i = 0; i < numSteps; i++) {
			...
			fos.println(tm + ", " + perfDataSets[0].getSeries(1).getValue(i) + ", " + perfDataSets[0].getSeries(0).getValue(i));
		}
		fos.close();
		return;
	}
	...
	private void updatePerfSeries() {
		...
		try {
			perfDataSets[0].getSeries(0).add(cts, ((NodeHWCNetwork)myNetwork).getSumDelay().getUpperBound());
			perfDataSets[0].getSeries(1).add(cts, ((NodeHWCNetwork)myNetwork).getSumDelay().getLowerBound());
		}
	...
	private void fillPerfPanel() {
		...
		dataset.addSeries(new TimeSeries("Worst Delay", Second.class));
		dataset.addSeries(new TimeSeries("Best Delay", Second.class));
